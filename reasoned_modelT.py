...
_NORMALIZED_PROPS = {rdfs_subclassof, SOME, VALUE, ONLY, EXACTLY, owl_onproperty, owl_onclass, owl_ondatarange, owl_withrestrictions}
...
class ReasonedModel(object):
  ...
  #CREATE $TEMP$ TABLE prop(s INTEGER NOT NULL, o INTEGER NOT NULL, l INTEGER NOT NULL);
  def prepare(self):
...
CREATE $TEMP$ TABLE prop(s INTEGER NOT NULL, p INTEGER NOT NULL, o INTEGER NOT NULL, l INTEGER NOT NULL);
...
"""
    ...
    #self.cursor.execute("""INSERT INTO prop SELECT s,o,1 FROM objs WHERE p=?""", (rdfs_subpropertyof,))
    #self.cursor.execute("""INSERT INTO prop SELECT s,o,1 FROM objs WHERE p=? AND o IN (?,?)""", (rdf_type, owl_object_property, owl_data_property))

    #self.cursor.execute("""INSERT INTO propSELECT s, p, o, 1 FROM objs WHERE p NOT IN (?, ?)  -- Exclude type and subclass relations AND p > 0 -- Only named properties""", (rdf_type, rdfs_subclassof))
    self.cursor.execute("""INSERT INTO prop SELECT s, p, o, 1 FROM objs WHERE p NOT IN (?, ?) AND p > 0""", (rdf_type, rdfs_subclassof))

    #self.cursor.execute("""INSERT INTO some SELECT p AS s, ? AS prop, o AS value FROM prop WHERE l = 1""", (SOME,))
    ...
    #self.cursor.execute("""CREATE UNIQUE INDEX prop_so ON prop(s,o)""")
    #self.cursor.execute("""CREATE INDEX prop_o ON prop(o)""")
    #self.cursor.execute("""CREATE INDEX prop_l ON prop(l)""")

    self.cursor.execute("""CREATE UNIQUE INDEX prop_spo ON prop(s, p, o)""")
    self.cursor.execute("""CREATE INDEX prop_p ON prop(p)""")
    self.cursor.execute("""CREATE INDEX prop_o ON prop(o)""")
    self.cursor.execute("""CREATE INDEX prop_l ON prop(l)""")    
...
""")
    ...
    for table, values in [("some", somes), ("data_value", data_values), ("only", onlys), ("exactly", exactly)]:
        if values:
  ...
  def _get_constructs(self, include_linked = 1):
    ...
    for table, p, has_card in [("some", SOME, False), ("only", ONLY, False), ("exactly", EXACTLY, True)]:
