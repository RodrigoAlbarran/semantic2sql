...
"""
        ...
        # Imports
        self.cursor.execute(
            """INSERT INTO types SELECT s,o FROM objs WHERE p=? AND o IN (?,?,?,?,?,?)""",
            (
                rdf_type,
        ...
        ########## Insert the queries Some and Only
        ...
        #print(self.cursor.execute("""SELECT s,prop,value FROM some""").fetchall())

        self.cursor.execute(
            """
            -- Insert existing SOME relations
            INSERT INTO some 
            SELECT 
                q1.o as value, 
                q2.o as prop,
                q3.s as s
            FROM objs q1 
                INNER JOIN objs q2 ON q1.s=q2.s 
                INNER JOIN objs q3 ON q2.s=q3.o 
            WHERE q1.p=? AND q2.p=?
            """,
            (SOME, VALUE)    
        )
...
""",
    ...
    def check_restriction_depth(self, depth):
        if depth <= self.max_restriction_depth:
            return True
...
"""
        ...
        repl = []
        for s, o in equivs:
            if (s in alreadys) or (o in alreadys):
                continue
            alreadys.add(s)
            alreadys.add(o)
            repl.append((s, o))
        ...
        for table_name in ("is_a", "types"):  # Reset due to removals
            self.last_inferences[self.rule_set.tables[table_name]] = (
                self.cursor.execute(
                    """SELECT MAX(rowid) FROM %s""" % table_name
                ).fetchone()[0]
            )
    ...
    ### 2. Ontology Interaction and Normalization ###
    ...
    def normalize_constructs(self):
        ...
        cursor.execute(
            """CREATE TEMPORARY TABLE tmpquads (s INTEGER, p INTEGER, o INTEGER)"""
        )
        cursor.execute(
            """INSERT INTO tmpquads SELECT s,p,o FROM quads WHERE s < 0 AND p IN (%s)"""
            % ",".join(str(p) for p in _NORMALIZED_PROPS)
        )
        l = []
        ...
        for flat_list in flat_lists:
            for rel in flat_list.flat.rels:
                r = cursor.execute(
                    """SELECT s,p,o FROM quads WHERE p=?""", (rel,)
                ).fetchall()
                if flat_list.flat.single_element:
                    ...
                    for s, p, o in r:
                        ...
                        if (len(rdf_list) == 1) and (
                            flat_list.rel in REMOVE_SINGLE_LIST_RELS
                        ):
                            cursor.execute(
                                """INSERT INTO is_a SELECT q1.s,?,1 FROM is_a q1 WHERE q1.o=?""",
                                (rdf_list[0][0], s),
                            )
        ...
        if debug:
            print("\nNORMALIZE STEP 0 :")
            for s, p, o in cursor.execute("""SELECT s,p,o FROM tmpquads""").fetchall():
                print("   ", s, p, o)
            print()

        cursor.execute(
            """CREATE TEMPORARY TABLE tmpnorm (def TEXT, s TEXT, s0 INTEGER, nb INTEGER)"""
        )
...
"""
            ...
            def dump1(self):
                ...
                for po, s, nb in cursor.execute(
                    """SELECT def, s, nb FROM tmpnorm"""
                ).fetchall():
                    print("   ", po, " | ", s, " | ", nb)
                print()

            def dump2(self):
                for s, p, o in cursor.execute(
                    """SELECT s,p,o FROM tmpquads"""
                ).fetchall():
                    print("   ", s, p, o)
                print()
            ...
            def insert(self):
                ...
                cursor.executemany(
                    "INSERT OR IGNORE INTO tmpquads VALUES (?,?,?)", self.insertions
                )
                ...
                nb = cursor.rowcount
                if debug:
                    print("NORMALIZE MERGES", self.merges)
                for s0, s in self.merges:
                    ...
                    cursor.executemany(
                        """UPDATE OR REPLACE tmpquads SET o=? WHERE o=?""", renames
                    )
                    cursor.executemany(
                        """UPDATE OR REPLACE is_a SET o=? WHERE o=?""", renames
                    )  # Optimizable
                    cursor.executemany(
                        """UPDATE OR REPLACE is_a SET s=? WHERE s=?""", renames
                    )
        ...
        while True:
            ...
            if not cursor.execute(
                """SELECT 1 FROM tmpnorm WHERE nb > 1 LIMIT 1"""
            ).fetchone():
                break

            for po, s0 in cursor.execute(
                """SELECT def, s0 FROM tmpnorm WHERE nb = 1"""
            ):
                for po1 in po.split(","):
                    insertions.insertions.append((s0, *int_p(po1.split(" "))))

            for po, s, s0 in cursor.execute(
                """SELECT def, s, s0 FROM tmpnorm WHERE nb > 1"""
            ).fetchall():
                for po1 in po.split(","):
                    insertions.insertions.append((s0, *int_p(po1.split(" "))))
        ...
        for po, s0 in self.cursor.execute("""SELECT def, s0 FROM tmpnorm"""):
            ...
            if po[0][0] in p_2_restriction_i:
                ...
                if type == SOME:
                    ...
                    del restriction[1]
                    onlys.append(restriction)
                elif type == MAX:
                    maxs.append(restriction)
                elif type == MIN:
                    mins.append(restriction)
                elif type == EXACTLY:
                    exactly.append(restriction)
                elif type == VALUE:
                    del restriction[1]
                    if (
                        self.world._get_by_storid(restriction[1])._owl_type
                        == owl_data_property
                    ):
...
"""
        ...
        for flat_list in flat_lists:
            ...
            cursor.execute(
                """CREATE %s TABLE %s(s INTEGER NOT NULL, o INTEGER NOT NULL)"""
                % (self.temporary, table_name)
            )
            cursor.execute(
                """CREATE UNIQUE INDEX %s_so ON %s(s,o)""" % (table_name, table_name)
            )

            self.sql_destroy += """DROP TABLE flat_lists_%s;\n""" % flat_list.rel

            cursor.execute(
                """INSERT OR IGNORE INTO %s SELECT s,o FROM tmpquads WHERE p=?"""
                % table_name,
                (flat_list.rel,),
            )
            nb += cursor.rowcount
            ...
            cursor.execute(
                """CREATE INDEX %s_os ON %s(o,s)""" % (table_name, table_name)
            )
        ...
        if self.explain:
            self.cursor.execute(
                """INSERT INTO explanations SELECT 'flat_lists_37', s, group_concat(o), 'assertion', '' FROM flat_lists_37 GROUP BY s"""
            )
        ...
        if owl_unionof in flat_list_rels:
            if owl_intersectionof in flat_list_rels:
                cursor.execute(
                    """CREATE %s VIEW flat_lists_292 AS SELECT * FROM flat_lists_30 UNION ALL SELECT * FROM flat_lists_31"""
                    % self.temporary
                )
            else:
                cursor.execute(
                    """CREATE %s VIEW flat_lists_292 AS SELECT * FROM flat_lists_30"""
                    % self.temporary
                )
        elif owl_intersectionof in flat_list_rels:
            cursor.execute(
                """CREATE %s VIEW flat_lists_292 AS SELECT * FROM flat_lists_31"""
                % self.temporary
            )
        else:
            cursor.execute(
                """CREATE %s TABLE flat_lists_292(s INTEGER, o INTEGER)"""
                % self.temporary
            )

        # Reset due to insertion / removal
        self.last_inferences[self.rule_set.tables["is_a"]] = self.cursor.execute(
            """SELECT MAX(rowid) FROM is_a"""
        ).fetchone()[0]
    ...
    def _get_constructs(self, include_linked=1):
        ...
        class Entity(object):
            ...
            def __str__(self):
                if self.name and not self.constructs:
                    return self.name
                l = []
                if self.name:
                    l.append(self.name)
                for construct in self.constructs:
                    l.append(str(construct))
                return "%s" % ",".join(l)

            def check(self):
                r = cursor.execute(
                    """SELECT 1 FROM objs WHERE s=? LIMIT 1""", (self.s,)
                ).fetchone()
                if not r:
                    self.is_new = True
            ...
            def __str__(self):
                ...
                try:
                    if self.xs[0] in (SOME, ONLY):
                        return "(%s %s %s)" % (
                            self.xs[2],
                            _restriction_type_2_label[self.xs[0]],
                            self.xs[3],
                        )
                    else:
                        return "(%s %s %s %s)" % (
        ...
        def get_entity(s):
            ...
            if not entity:
                if s > 0:
                    name = self.world._unabbreviate(s).rsplit("#", 1)[-1]
                else:
                    name = None
                entity = s_2_entity[s] = Entity(s, name)
            return entity

        for table, construct_class in [
            ("flat_lists_30", OrConstruct),
            ("flat_lists_31", AndConstruct),
            ("flat_lists_37", DisjointConstruct),
            ("flat_lists_87", NotConstruct),
        ]:
            for s, os in self.cursor.execute(
                """SELECT s, group_concat(o) FROM %s GROUP BY s""" % table
            ):
                entity = get_entity(s)
                entity.constructs.append(
                    construct_class([get_entity(int(o)) for o in os.split(",")])
                )

        for table, p, has_card in [
            ("some", SOME, False),
            ("only", ONLY, False),
            ("max", MAX, True),
            ("min", MIN, True),
            ...
            for r in self.cursor.execute("""SELECT * FROM %s""" % table):
                ...
                entity = get_entity(s)
                entity.constructs.append(
                    RestrictionConstruct([p, card, get_entity(prop), get_entity(value)])
                )
    ...
    def dump_constructs(self):
        ...
        for s in reversed(sorted(s_2_construct)):
            ...
            s = "  %s : %s" % (s, r)
            if construct.has_superclass:
                s = "%s (has superclass)" % s

            if construct.is_new:
                print(s)
            else:
                print("\x1b[2m%s\x1b[0m" % s)
        ...
        for table in self.rule_set.tables.values():
            ...
            for row in self.cursor.execute(
                ...
                for i in range(len(row)):
                    if row[i] is None:
                        pass
                    elif not isinstance(row[i], int):
                        pass
                    else:
                        if (table.name == "is_a") and (i == 2):
                            pass
                        elif table.name.startswith("clause_") and (i == 2):
                            pass  # clause nb
                        elif row[i] > 0:
                            ...
                            if construct:
                                row[i] = "%s\x1b[2m:%s\x1b[0m" % (
                                    row[i],
                                    str(construct).replace("onto.", ""),
                                )
                            else:
                                row[i] = "%s" % (row[i])
                ...
                if self.explain and (table.name == "is_a"):
                    ...
                    for (rule,) in self.cursor.execute(
                        """SELECT rule FROM explanations WHERE s=? AND o=?""",
                        (orig_row[0], orig_row[1]),
                    ):
                        if not rule in rule_2_nb:
                            rules.append(rule)
                        rule_2_nb[rule] += 1
                    l = []
                    for rule in rules:
                        if rule_2_nb[rule] == 1:
                            ...
                            l.append("%s x%s" % (rule, rule_2_nb[rule]))
                    row.append(
                        "   \x1b[2m%s : %s\x1b[0m"
                        % (sum(rule_2_nb.values()), ", ".join(l))
                    )
    ...
    def execute_stage(self, stage):
        ...
        self._candidate_completions = set(
            stage.initial_completions
        )  # Need to define it before running preprocesses, because preprocesses may add candidates!
    ...
    def run(self, x=None, debug=1):
        ...
        if locked:
            self.world.graph.release_write_lock()  # Not needed during reasoning
        try:
            ...
            for table in self.rule_set.tables.values():
                if not self.rule_set.table_2_inferrable.get(table, None):
                    continue
                try:
                    nb = (
                        self.cursor.execute(
                            """SELECT MAX(rowid) FROM %s""" % table.name
                        ).fetchone()[0]
                        or 0
                    )
            ...
            for stage in self.rule_set.stages:
                if stage is self.rule_set.stages[-1]:
                    self._optimize()
                self.execute_stage(stage)

            t = time.time()
            self.new_parents, self.new_equivs, self.entity_2_type = (
                self.extract_inferences()
            )
            self.extract_result_time = time.time() - t

        finally:
            if locked:
                self.world.graph.acquire_write_lock()  # re-lock when applying results

        if isinstance(x, Ontology):
            ontology = x
        if CURRENT_NAMESPACES.get():
            ontology = CURRENT_NAMESPACES.get()[-1].ontology
        else:
            ontology = self.world.get_ontology(_INFERRENCES_ONTOLOGY)
    ...
    def execute_rule(self, rule):
        ...
        if rule.recursive:
            ...
            while True:
                ...
                self._check_optimize_table(
                    rule.table, self.last_inferences[rule.table] + nb_new_triples
                )
            ...
            if nb_new_triples:
                self.compute_completion_candidates(
                    rule, nb_new_triples, added_nb_inferences
                )
            ...
            if result is None:
                nb_new_triples, added_nb_inferences = (
                    0,
                    0,
                )  # or whatever default values make sense
            else:
        ...
        if self.debug:
            if nb_new_triples:
                print(
                    "\x1b[1m %0.4fs," % t,
                    "produced %s matches and %s hits\x1b[0m"
                    % (rule.total_matches - current_matches, nb_new_triples),
                )
            else:
                print(
                    "\x1b[2m %0.4fs," % t,
                    "produced %s matches and 0 hit\x1b[0m"
                    % (rule.total_matches - current_matches),
                )
            self.check_last_inferences()
        ...
        for table in self.rule_set.tables.values():
            if table in self.last_inferences:
                ...
                if self.last_inferences[table] != nb:
                    raise RuntimeError(
                        "Inference count lost for table %s! count is %s, real is %s."
                        % (table, self.last_inferences[table], nb)
                    )

    def extract_inferences(self):
        # Infer equivalence from double inheritance
        self.cursor.execute(
            """CREATE TABLE equiv (s INTEGER NOT NULL, o INTEGER NOT NULL)"""
        )
...
""",
        ...
        o_2_type = {
            ...
            owl_data_property: "property",
            owl_named_individual: "individual",
        }
        entities = set(new_parents)
        entities.update(new_equivs)
        for s in entities:
            entity_2_type[s] = o_2_type[
                (
                    self.cursor.execute(
                        """SELECT o FROM types WHERE s=? LIMIT 1""", (s,)
                    ).fetchone()
                    or (owl_class,)
                )[0]
            ]
    ...
    def print_rule_usage(self):
        for stage in self.rule_set.stages:
            ...
            for rule_type, rules in [
                ("Preprocess", stage.preprocesses),
                ("Completion", stage.completions),
            ]:
                if not rules:
                    continue
                ...
                for rule in sorted(rules, key=lambda rule: -rule.total_time):
                    if self.debug:
                        matches = "%s matches and " % rule.total_matches
                    else:
                        matches = ""
                    if rule_type == "Preprocess":
                        nb_execution = ""
                    else:
                        nb_execution = "%s executions, " % rule.nb_execution
                    if rule.total_hits:
                        start = "\x1b[1m"
                    else:
                        start = "\x1b[2m"
                    if self.debug and hasattr(rule, "search_time"):
                        search_time = " (%0.4fs for SQL)" % rule.search_time
                    else:
                        search_time = ""
                    print(
                        "%s    %s: %0.4fs%s, %sproduced %s%s hits\x1b[0m"
                        % (
                            start,
                            rule.name,
                            rule.total_time,
        ...
        print(
            "Extraction des resultats: %0.4fs" % self.extract_result_time,
            file=sys.stderr,
        )
